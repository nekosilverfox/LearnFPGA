...
它由一只 PMOS 和一只 NMOS 组成：

- 输入为低电平时：PMOS 导通、NMOS 截止，输出为高电平。
- 输入为高电平时：PMOS 截止、NMOS 导通，输出为低电平。
...
- **反相器**：更常用于电路分析，强调电压反相、传输特性、延迟和功耗。
- **反门/非门**：更常用于数字逻辑，强调逻辑功能。
...
> **P 管要比 N管的栅极宽度大：**
>
> 因为在 CMOS 工艺中，**电子的迁移率比空穴高**，所以在尺寸相同、偏置相近时：
>
> - NMOS 依靠电子导电，导通能力较强；
> - PMOS 依靠空穴导电，导通能力较弱。
>
> MOS 管的驱动能力大致与下面的参数成正比：
>
> $$
> \beta=\mu C_{\mathrm{ox}}\frac{W}{L}
> $$
>
> 其中 **$\mu$** **是载流子迁移率**，$W$ 是沟道宽度，$L$ 是沟道长度。通常有：
>
> $$
> \mu_n>\mu_p
> $$
>
> 因此，为了让 PMOS 和 NMOS 的驱动能力接近，需要把 PMOS 做得更宽：
>
> $$
> \mu_p\frac{W_p}{L_p}\approx \mu_n\frac{W_n}{L_n}
> $$
>
> 当两者沟道长度相同时：
>
...
>
> $$
> W_p\approx 2W_n\sim 3W_n
> $$
>
> 这样做主要有两个目的：
>
> 1. **使输出上升和下降速度接近**  
>     PMOS负责给负载电容充电，NMOS负责放电。若尺寸相同，PMOS充电较慢，输出上升时间会明显大于下降时间。
> 2. **使反相器的翻转点接近电源电压的一半**  
>     即让输入约为 $V_{DD}/2$ 时，PMOS和NMOS的电流大致相等，提高高、低电平噪声容限的对称性。
>
> 这里所说的“栅极宽度大”，通常是指版图中的**沟道宽度 (W)**  更大，并不是栅氧化层更厚，也不是栅极长度 (L) 更长。具体宽度比取决于工艺参数，不一定严格等于2或3。
...
- **底层构成**：由边沿触发触发器组成，是时序电路**最基础的存储单元**（不是门电路基本单元，门电路基本单元是MOS/逻辑门）
- **触发方式**：时钟边沿触发，无电平透明期，状态稳定、无毛刺
- **速度特性**：速度最快，几乎无读写延迟，可随时读写、无访问限制
- **硬件特点**：占用芯片面积大、成本高、容量极小
- **用途**：用于**时序逻辑核心状态保存**，如计数器、状态机、流水线、变量暂存
- **总结**：FPGA/CPU 最核心、最高速、最小容量的片上存储

‍

**静态随机存储器（SRAM：Static Random Access Memory）**

- **底层构成**：由6个MOS管构成的存储单元，无需刷新
- **特性**：只要不断电，数据永久保持（静态存储）
- **速度**：高速，仅次于寄存器
- **硬件特点**：单元面积比寄存器小，比DRAM大；功耗较高
- **访问规则**：同一时刻**只能读写一个地址数据**
- **用途**：FPGA片上内存、Cache缓存、临时大批量数据缓存
- **总结**：用于**片上大容量、高速临时存储**
...
- **底层构成**：电容+单晶体管结构，靠电容电荷存数据
- **核心特点**：电容漏电，**必须周期性刷新**（动态存储）
- **速度**：速度低于 SRAM、寄存器
- **硬件特点**：单元面积极小、容量极大、成本极低
- **读写特性**：读写带宽大、吞吐量高，按地址块访问
- **位置**：多为**片外内存**（电脑内存条），部分高端芯片内置
- **总结**：低速、超大容量、用于系统主存

‍

**非易失存储器（NVM: Non-Volatile Memory / 掉电保存）**
...
- 控制端**高**电平（高有效锁存器）：电路**透明直通**，输入是什么，输出**<u>立刻</u>**跟着变；
...
2. <span data-type="text" style="background-color: var(--b3-card-success-background); color: var(--b3-card-success-color);">电平有效期间输入变化会直接渗透到输出，容易产生毛刺、竞争冒险；</span>
...
> 参考文档：
>
> https://zhuanlan.zhihu.com/p/558543678
>
> 【代码】https://vlab.ustc.edu.cn/guide/doc_simple_timing.html

**SR 锁存器是 FPGA 里最简单的【状态保持电路】** 示波器开发里：**触发锁存、标志位锁定、异常状态捕获** 全是基于它的逻辑衍生的。
...
1. **S**  **=**  **Set 置位**：强制输出拉高 Q\=1

    S：置位信号（事件触发、拉高锁 1）
2. **R**  **=**  **Reset 复位**：强制输出拉低 Q\=0
...
1. **或****非门构成 SR 锁存器**：<span data-type="text" style="color: var(--b3-font-color11);">高</span>电平有效（S/R 给 1 才生效，课堂最常用）
2. **与****非门构成 SR 锁存器**：<span data-type="text" style="background-color: var(--b3-card-warning-background); color: var(--b3-card-warning-color);">低</span>电平有效（输入带横线 $\overline{S}、\overline{R}$，芯片里多见）

‍

|S|R|Q|Q'|||
| ---| ---| ---| ----| -----------------| --------------------------------------------------------|
|0|0|Q|Q'|保持 / 锁存状态|Q、Q'维持上一次的结果==【输入全部无效，保存之前的数据】==|
|0|1|0|1|复位状态|把锁存器清零，记住数值 0|
|1|0|1|0|置位状态|把锁存器设置成 1，记住数值 1|
|1|1|1|1|非法禁止状态|此时 Q\=1，Q'\=1，两个输出不再互补，逻辑矛盾<br />==一旦同时撤去 S、R，输出状态会随机跳变，无法预测==​**==实际电路严禁出现这种输入组合==**<br />|

把 SR 锁存器想象成一个记忆小盒子：

- 按 S 按钮 (S\=1)：盒子里固定装数字 1，松手 (S\=0) 数字不变
- 按 R 按钮 (R\=1)：盒子清空变成 0，松手 (R\=0) 数字不变
- S、R 都不按：盒子里数字不变，锁住数据
- S、R 一起按住：盒子逻辑错乱，<span data-type="text" style="background-color: var(--b3-card-success-background); color: var(--b3-card-success-color);">不允许这么操作</span>
...
1. 先输入 S\=1,R\=0 → Q\=1
2. 改成 S\=0,R\=0 → Q 保持 1 不变（锁住 1）
3. 再输入 S\=0,R\=1 → Q 变成 0
4. 再改成 S\=0,R\=0 → Q 保持 0 不变（锁住 0）
...
**==D 锁存器（D 源于”Data”）==** ，也叫作**透明锁存器**，是门控 SR 锁存器的简单扩展，其消除了无效输入状态（亚稳态）的可能性。

由于门控 SR 锁存器允许我们在不使用 S 或 R 输入的情况下锁存输出，我们可以通过使用互补驱动器驱动设置和复位输入来移除其中一个输入，即我们移除一个输入并自动使其成为另一个输入的互补输入。

![image](image74.png)

![image](image75.png)

- 只要 **==Enable 线为高电平==**，D 锁存器就会**==输出==** D 输入，
- 否则（**Enable 线为低电平**）输出是当 Enable 输入为最后一个高电平时 D 输入的输出。这就是为什么它也被称为**透明锁存器** - ==当 Enable 被置位时，锁存器被称为“透明” - 它信号直接传播通过它，好像它不存在一样==。

D 锁存器可以在行为建模中建模，如下所示：

```verilog
module D_latch_behavior (
	input D, input Enable, 
...
SR触发器可能被毛刺触发，这个毛刺是什么，哪里来的？

‍

一、​**毛刺是什么（适配示波器 FPGA 开发）**

毛刺（glitch / 竞争冒险脉冲）：**信号本应稳定在 0 或 1，却出现极窄、无意义的瞬时电平**​**==跳变==**​**脉冲**，宽度通常只有几 ns 甚至几百 ps。比如一根稳定高电平的信号线，中间瞬间跌落一下 0，立刻回弹回 1，这个向下的窄脉冲就是毛刺；稳定低电平中间短暂跳 1，也是毛刺。

SR 锁存器是**电平敏感异步电路**，只要 S/R 输入出现哪怕极短的有效电平毛刺，就会误改存储状态，这是示波器触发电路必须规避的坑。

‍

**二、毛刺两大核心来源（分组合逻辑硬件、外部模拟信号两类，对应示波器业务）**

来源 1：数字逻辑内部竞争冒险（FPGA 布线、门电路延迟产生，最常见）

同一逻辑信号走不同路径到达门电路，路径上金属线长短、门级数不同，信号传输延迟不一样，到达输入端存在时间差，输出瞬间产生错误窄脉冲。

举 SR 锁存配套逻辑场景：用组合逻辑生成 S 置位信号，逻辑表达式 S \= A · `A'`，==理论输出永远 0；但 A 经过非门有延迟，A 和== ==​`A'`​==​  ==**不会**==​==同步到达与门，极短时间内两者同时为 1，S 输出一个正向毛刺==。这个毛刺送到 SR 锁存器 S 端，会误置位 Q，示波器会**虚假触发、乱采集波形**。

‍

**来源 1：FPGA 内部产生场景**

1. 多层逻辑门串联、布线绕线过长，信号延迟不一致
2. 多路信号译码、编码器、多路选择器输出（147/148/153 这类组合器件极易出毛刺）
3. Vivado 综合 / 布局布线后，逻辑路径延迟失衡

‍

**来源 2：外部模拟 / 采样电路耦合干扰（示波器开发重点，高频场景严重）**

示波器前端是高速模拟采样通道，数字控制走线和模拟高速信号线近距离并行，极易耦合噪声毛刺：

1. 电磁串扰：相邻高速差分采样线、时钟线跳变，通过寄生电容 / 电感耦合到 S、R 数字控制线，产生窄毛刺；
2. 电源噪声：模拟电源、数字电源地平面分割不完善，采样瞬间电源波动，叠加到逻辑输入；
3. 前端运放、ADC 输出噪声：模拟量化噪声，超过逻辑阈值就会形成毛刺；
4. 按键、外部触发输入长线：长线接收空间电磁干扰，引入脉冲毛刺。

‍

**来源 3：信号边沿畸变（高速电路特有）**

高频时钟、触发信号边沿过陡，信号反射、阻抗不匹配，出现振铃、过冲，振铃的尖峰跨过高低电平阈值，等效产生毛刺。

​

**三、为什么 SR 锁存器最怕毛刺**

1. SR 锁存**==无时钟同步机制==**，全程实时监测 S、R 电平，==只要==出现有效电平（高有效 SR 只要 S\=1 一瞬间），立刻修改存储状态；
2. ==毛刺宽度再窄（几百 ps），只要满足 MOS 管导通建立时间，锁存器就会捕获这个虚假信号==

    > MOS 管导通建立时间（从驱动到完全导通），主流器件在 5–50 ns 区间；FPGA 内部 CMOS 管更快，约 100 ps–1 ns 量级。
    >
3. 示波器单次触发逻辑依赖 SR 锁存保存 “触发标志”，毛刺误触发会导致设备无波形乱采集、界面异常、误捕获噪声。

对比：边沿 D 触发器只在时钟跳变瞬间采样，其余时间输入毛刺完全无效，这也是 FPGA 时序触发电路统一用寄存器而不用原生 SR 锁存的核心原因。

‍

**四、工程缓解思路（示波器 FPGA 开发实用方案）**

1. 异步 SR 逻辑替换为**同步时序 D 触发器**，用系统时钟同步采样输入，滤除时钟间隙内的毛刺；
2. 组合逻辑输出增加寄存器打拍，同步消除竞争冒险带来的毛刺；
3. 外部触发输入增加多级同步寄存器、数字滤波（连续多周期稳定才判定有效）；
4. PCB 层面分离模拟采样走线与数字控制走线，减小串扰，完整分割数字 / 模拟地；
5. 控制信号增加 RC 硬件滤波，滤除 ps/ns 级窄毛刺。

---

‍

### 触发器（Flip-flop，==边沿==触发基础存储）

**1. 定义**

在锁存器基础上改良，**只识别信号****<u>跳变沿</u>**，仅在一瞬间更新状态，其余全程保持数值。触发沿分两种：
...
一个触发器内部是**两级 D 锁存器**​**级联**：第一级主锁存器在时钟低电平接收数据，第二级从锁存器在时钟**上升沿**瞬间传递数值，平时全部锁死隔绝输入干扰。
...
**带时钟沿触发的**​**<u>触发器</u>**​**封装单元**，工业、FPGA、数字芯片标准叫法：单个边沿 D 触发器就是 1bit 寄存器；多个触发器并排组成多位寄存器（如 8 位、32 位寄存器）。*课本原话：寄存器 = 沿触发触发器*
...
1. 锁存器≠触发器
    ...
    锁存看**一段****电平**，触发器只看**一瞬间跳变**；

2. Verilog 里如果组合逻辑漏写 else、没给 reg 赋初值，会被 Vivado 综合成锁存器（bug 源头）；工程同步时序全部要用寄存器（边沿触发器）；

3. 74 系列芯片：74LS373 是 D 锁存器，74LS374 是 8 位寄存器（8 个上升沿 D 触发器），直观对比两者硬件差异。
...
> https://semiconductor.samsung.cn/support/tools-resources/dictionary/semiconductor-glossary-fin-field-effect-transistor-finfet-process/

该技术在Xilinx UltraScale+ 系列中被采用

鳍式场效应晶体管（FinFET）工艺是旨在突破传统平面（2D）结构限制的3D工艺。

==由于结构看起来像鱼鳍，因此被称为“鳍（Fin）式”FET==。晶体管中，当电压施加到栅极时，电流通过沟道从“源极”流向“漏极”。==栅极与沟道之间的接触面积越宽，效率就越高。==鳍式场效应晶体管（FinFET）工艺采用鳍状3D结构增加接触面积，从而提高了半导体性能并减少电流泄漏。

==右侧的图像，把源极和漏极像积木一样拔高嵌入到栅极当中==

 三星半导体从14nm开始采用鳍式场效应晶体管（FinFET）工艺技术。2016年10月，三星首次量产10nm鳍式场效应晶体管（FinFET）。
...
在 ADC（模数转换器）研究里，**杂散**通常指频谱中那些本来不应该出现、但却以明显“尖峰”形式出现的离散频率分量，英文常叫 **spur** 或 **spurious tone**。

例如，给 ADC 输入一个非常纯净的 10 MHz 正弦波，理想情况下做 FFT 后，频谱里主要只有 10 MHz 这一根谱线。但实际测量时，可能还会看到：

- 20 MHz、30 MHz 等位置的尖峰：通常是二次、三次谐波；
- 8 MHz、12 MHz 等附近的尖峰：可能来自时钟、干扰或混频；
- 某些固定频率的小尖峰：可能由电源纹波、数字串扰或 ADC 内部失配造成。

这些额外的离散尖峰，都可以广义地称为杂散。
...
- **噪声**像整体铺在地面上的“草”，通常是连续、分散的；
- **杂散**像突然冒出来的“电线杆”，集中在某个特定频率上。

所以，杂散虽然有时幅度不大，但因为它集中在一个频点，可能特别影响通信、雷达和高精度测量。
...
1. **ADC 非线性**  
    输入正弦波经过非线性后，会产生二次、三次等谐波。
2. **采样时钟问题**  
    时钟中混入其他频率，或时钟受到调制，会产生额外谱线。
3. **电源干扰**  
    例如 50 Hz、100 Hz 或开关电源频率进入 ADC。
4. **数字电路串扰**  
    FPGA、SPI、数据总线等数字信号耦合到模拟输入或参考电压。
5. **交织 ADC 通道失配**  
    在时间交织 ADC 中，通道之间的增益、偏置和采样时刻不一致，会产生特定位置的杂散。
6. **输入信号源本身不纯**  
    有时看到的杂散不是 ADC 产生的，而是信号发生器、放大器或滤波器带来的。

‍

**==杂散通常怎么衡量？==**

最常用的指标是 **SFDR，Spurious-Free Dynamic Range，无杂散动态范围**。

它表示：

> 输入主信号幅度，与频谱中最大杂散幅度之间的差值。
...
$$
SFDR = -1-(-81)=80\ \text{dBc}
$$

SFDR 越大，说明最大杂散越低，ADC 的频谱纯净度越好。

需要注意，**杂散是一个统称，谐波是杂散的一种**。在 ADC 数据手册或测试中，最大杂散可能是谐波，也可能是其他非谐波分量。
...
这张图表示：**仪器 C 的模拟输入端接 0 V 直流时，对 ADC 输出数据做 FFT 得到的频谱**。

理想情况下，输入为 0 V 时，频谱中应该主要只有随机噪声；但图中出现了大量又窄又高、近似等间距排列的尖峰。这些尖峰就是论文所说的**杂散**。

在时间交织 ADC 中，假设内部有 $M$ 个子 ADC，每个子 ADC 都存在不同的直流偏置 $b_0,b_1,\dots,b_{M-1}$。输入为零时，输出可以写成：

$$
y[n]=b_{n\bmod M}
$$

这个偏置序列每 $M$ 个采样点重复一次，因此会在频域中产生梳状谱线：

$$
f_k=k\frac{F_s}{M},\qquad k=0,1,2,\dots
$$

**图中**相邻尖峰的间隔约为：

$$
\Delta f=250\ \mathrm{MHz}=0.25\ \mathrm{GHz}
$$
...
$$
M=\frac{F_s}{\Delta f}
=\frac{20\ \mathrm{GHz}}{0.25\ \mathrm{GHz}}
=80
$$

需要注意两点：

- **杂散就是图中高出噪声底的细长竖直尖峰**，不是下面连续的黑色“厚带”。
- 不是“一个尖峰对应一个 ADC”，而是**尖峰间距反映偏置误差序列的重复周期**，由此反推出子 ADC 数量。

图在 $10\ \mathrm{GHz}$ 左右出现的宽阔下凹主要是整体噪声包络或系统频率响应的变化，不是论文用来计算 $M$ 的关键；关键是那些每隔约 $0.25\ \mathrm{GHz}$ 重复出现的窄尖峰。
...
当用示波器测量抖动时，一定要知道测试设备对测量结果的贡献。您测量的始终是示波器中的固有抖动与被测器件及DUT驱动源所造成抖动的组合。通常DUT造成的抖动与测试系统的固有抖动有同一量级的幅度。

在多数情况下，这三个贡献者是随机和不相关的，因此如果您使用统计法测量抖动（有效值抖动），它们就以各贡献者的均方根叠加。有可能测量示波器和源的贡献，从总测量结果中抽出DUT的贡献。

有许多因素影响示波器的实际抖动贡献，而且仪器间也各不相同。

1. 触发电平设置：对最小抖动的最佳触发电平，各种触发电路并不相同。它会随器件的不同而不同。调整触发电平，以得到最小的抖动。
2. 触发信号上升时间：示波器测量触发信号和数据间的相对抖动。不能看到触发和数据信号上的共同抖动。一般来说，触发信号上升时间越慢，抖动就越高。在同一频率时，方波触发信号的抖动比正弦波触发信号小。用限幅放大器“整形”触发信号。
3. 触发信号幅度：触发电路的灵敏阈值随器件而不同。越接近阈值电平的触发幅度得到越高的抖动。应使用尽可能高幅度的触发信号。
4. 触发和数据采样间的延迟：定时不确定度加长了时基从触发事件到实际采样的等待。要保证各次测量触发信号与数据间延迟的一致性。在54120A, 54750A, 83480A, 86100A中有相对延迟设置的抖动指标（2.5 psec rms + (delay setting)\* 5E-5）。
5. 所取样本数：对于任何像抖动这样的随机事件，表征精度随样本数增加而增加。测量要使用一致的样本数，应记住对于真随机高斯分布，理论峰峰抖动是无限大。
...
由于垂直噪声能影响抖动测量。所以很难确定或无法确定信号边缘的迹线厚度是由抖动还是由垂直噪声所引起的。

这通常非常有助于查看波形中平坦的部分，因为波形中的振幅相对较连续而且迹线厚度很可能是由噪声所致。将该迹线厚度与信号边缘的迹线厚度相比较可发现，您所测量的抖动实际有多大程度上是由垂直噪声引起的。

您可以使用取平均值的方法来实际消除垂直噪声（尽管这样也会消除随机抖动）。在54800系列Infiniium示波器中打开平均值，点击**Setup | Acquisition（设置|采集）** ，在**Averaging（取平均值）** 选项下选择**Enabled（激活）** ，并输入适当的平均值# 。如果您想快速从主要波形显示窗口中打开和关闭平均值，只需在显示窗口中点击右键并选择或取消选择**Averaging（取平均值）** 即可。

‍

### 在使用外部触发器时有多大抖动？

除非触发信号与任意波形发生器采样时钟同步，否则您将看到抖动等于内部任意波形发生器的采样周期（10 ns），对于 N6030A 来说等于 8 个采样周期（6.4 ns）。

内部任意波形发生器需要选件 HEC 来支持外部时钟输入。
...
内部任意波形触发延迟源于自由运行的振荡器，使用任意非零延迟都可以保证抖动。

C.04.60 (ESG) 和 C.04.61 (PSG) 提供的固件补丁可以时间或时钟采样的形式描述延迟。以时钟采样形式描述延迟后，该延迟可以同步至任意波形采样时钟，从而消除自由运行的振荡器引发的抖动。
...
> [【0】宽带时间交织数据采集系统中高吞吐率低复杂度并行校准方法.pdf](assets/【0】宽带时间交织数据采集系统中高吞吐率低复杂度并行校准方法-20260714084128-s4x88st.pdf)

先用一句话理解 TIADC 校准

**TIADC 校准，就是让多个轮流采样的 ADC 通道，表现得像同一个完全一致的高速 ADC。**

16 路 TIADC 并不是 16 路信号，而是把同一个高速模拟信号分给 16 个 ADC，让它们在不同时间点轮流采样，再把数据按照时间顺序拼起来。

---

**一、16 路 TIADC 是什么**

假设系统总采样率为 (F\_s)，那么每个 ADC 只需要工作在：

$$
F_{\text{sub}}=\frac{F_s}{16}
$$
...
- $(m=0,1,\ldots,15)$，代表第几路 ADC；
- (k) 代表该通道的第几次采样；
- (T\_s\=1/F\_s)，是整个系统的采样间隔。

例如总采样率是 16 GSPS，每个 ADC 只需要采样 1 GSPS：
...
论文中的数学模型也是这个思路：M 个子 ADC 各自以 $(F_s/M)$ 采样，拼接后得到总采样率 (F\_s)。论文把公共模拟路径的频率响应记为 $(H(j\Omega))$，各子 ADC 独立路径的响应记为 $(G_m(j\Omega))$。

---

**二、为什么必须校准**

理想情况下，16 个 ADC 应该完全相同。但现实中，每一路的器件、走线、时钟和温度都不可能完全一致。

主要误差可以先理解成下面四种：

|误差|通俗理解|典型影响|
| ----------------------| ------------------------------------| ------------------------------------|
|偏置误差 Offset|有的通道零点偏高，有的偏低|没有输入时输出不为零，产生固定杂散|
|增益误差 Gain|同样大小的信号，各路测出来不一样大|拼接波形忽高忽低，产生镜像杂散|
|时间误差 Timing skew|某一路采得早一点或晚一点|输入频率越高，误差越严重|
|频率响应误差|每路对不同频率的增益和相位不同|宽带信号失真，产生复杂杂散|

可以把 16 路 ADC 想成 16 个人接力拍照：

- 有人相机本身有底色，是偏置误差；
- 有人把照片拍亮了，是增益误差；
- 有人按快门早了或晚了，是时间误差；
- 有人的相机在低频正常、高频模糊，是**频率响应误差**。

校准就是先找出每个人的差异，再把照片统一修正。

---

**三、你说的“四种方式”并不是完全同一层级**

这里需要先纠正一个很容易混淆的地方：

- **前台校准、后台校准、后台实时校准**，主要描述“什么时候识别误差、什么时候更新参数”。
- **频响误差校准**，描述“要校准什么误差”。

因此，频响误差校准既可以采用前台方式，也可能采用后台方式；它并不是与前三者完全并列的第四种运行模式。
...
设备正式采集信号前，先暂停正常工作，给 TIADC 输入一个已知的标准信号，例如：
...
系统根据“输入本来应该是什么”和“16 路 ADC 实际测到了什么”，计算每一路的误差参数。

例如测得：

```text
ADC0：作为参考
ADC1：增益高了 0.3%，采样晚了 0.8 ps
ADC2：偏置多了 2 个码，采样早了 1.2 ps
……
ADC15：高频段衰减比其他通道更明显
```

这些参数被存入 FPGA、寄存器或存储器中。正式工作后，系统只负责按照参数修正数据。
...
- 校准时通常不能正常采集；
- 温度、供电和器件老化发生变化后，原来的参数可能不再准确；
- 需要定期开机校准或重新校准。

### 适用场景

实验室示波器、仪器开机自检、工作温度比较稳定的采集设备，以及宽带频响扫频校准。

---

## 2. 后台校准

英文通常叫 Background Calibration。

‍

**它是什么意思**

设备不停止正常采集，在采集业务信号的同时，悄悄估计各通道误差并更新校准参数。
...
<span data-type="text" style="background-color: var(--b3-card-success-background); color: var(--b3-card-success-color);">不</span>额外加入校准信号，而是利用正常输入信号的统计特征，例如：

- 各路输出均值理论上应该接近；
- 各路输出功率应该接近；
- 相邻通道之间应具有一定相关性；
- 拼接后不应该出现 $(F_s/16)$ 周期性的杂散；
- 通过杂散幅度反推增益或时间误差。

例如发现 ADC3 的平均输出总比其他路高，就逐渐调整 ADC3 的偏置补偿值。

**第二类：注入式后台校准**

向正常信号中加入一个幅度很小、频率已知的校准信号，然后从 ADC 输出中提取这个校准分量，判断各通道误差。

业务信号仍然正常采集，但系统中存在一条“校准参考”。
...
- 算法通常比前台校准复杂；
- 校准结果可能依赖输入信号特性；
- 输入信号太单一、幅度太小或频谱不充分时，某些误差可能无法准确估计；
- 参数更新过快可能引起震荡，更新过慢又跟不上环境变化；
- 注入信号可能轻微影响正常信号。
...
对于 16 路高速 TIADC，数据量非常大。假如 FPGA 时钟只有 250 MHz，而输入数据达到数 GSPS，FPGA 每个时钟周期就必须并行处理多组采样点，因此需要多路并行滤波、流水线处理、FFT/IFFT 和大量 DSP 资源。
...
这种系统的<span data-type="text" style="background-color: var(--b3-card-error-background); color: var(--b3-card-error-color);">补偿过程是实时的</span>，但<span data-type="text" style="background-color: var(--b3-card-success-background); color: var(--b3-card-success-color);">参数</span><span data-type="text" style="background-color: var(--b3-card-error-background); color: var(--b3-card-error-color);">并没有在后台实时估计和更新</span>，所以它更准确的称呼是：

> 预校准参数 + 实时数字补偿

而真正的后台实时校准应该包含：

> 在线参数辨识 + 在线参数更新 + 在线实时补偿

‍

**这篇论文属于哪一种**

从论文给出的内容看，它重点解决的是**校准滤波器如何在 FPGA 上以高吞吐率实时运行**，而不是重点研究运行期间如何从未知业务信号中实时辨识误差。

论文使用 AWLS 方法设计低阶 FIR 滤波器，并通过频谱折叠减少 PTV 滤波器的 IFFT 并行路数，最终在 FPGA 上实现 FRM、FRI 两个校准模块的 8 路并行处理，达到 2 Gpts/s 吞吐率。

所以更准确地说，这篇论文主要研究的是：

> **预先获得频响信息和滤波器系数，然后在 FPGA 中进行高速、实时、并行补偿。**

它具有“实时校准数据”的能力，但论文并没有把重点放在典型的“后台在线自适应误差辨识”上。

---

## 4. 频响误差校准

频响误差校准解决的不是“一个固定增益差”，而是：

> 同一个通道在不同输入频率下，增益和相位表现不同；不同通道的频率响应也不相同。

例如 ADC1 和 ADC2 在低频时几乎一致：

```text
100 MHz：ADC1 = 1.000，ADC2 = 0.999
```

但到了高频：

```text
7 GHz：ADC1 = 0.92，ADC2 = 0.80
```

这时用一个固定增益系数无法同时校准低频和高频，必须使用滤波器对每个频段分别补偿。
...
> Frequency Response Imperfection

它表示整个采集系统的公共通道频率响应不够平坦。
...
虽然输入各频率幅度一样，但系统输出不一样，这就是幅频响应起伏。

论文用公共路径 (H(j\\Omega)) 表示这种频响，理想补偿器是它的逆：

[  
C(j\\omega)\=\\frac{1}{H(j\\omega)}  
]

直观理解：

- 原系统在 3 GHz 衰减，就让校准滤波器在 3 GHz 增强；
- 原系统在 7 GHz 增强，就让校准滤波器在 7 GHz 衰减；
- 两者相乘后，整体频率响应尽量变平。

论文使用 FIR 滤波器校准 FRI，并通过 AWLS 方法把 FIR 阶数降低，从而节省 FPGA 的 DSP 资源。论文明确说明，FRI 是通道自身通带波动导致不同频点增益不同，需要使用 FIR 滤波器校准。

实验中，系统原来的幅频响应波动约为 4.3 dB，校准后控制到约 ±0.25 dB。
...
它表示 16 路 ADC 的独立路径 (G\_m(j\\Omega)) 不相同。

需要注意，FRM 不仅是幅度不同，还包括：
...
论文指出，各子 ADC 前端路径的差异会引入随频率变化的增益和相位失配，并在拼接频谱中产生镜像和杂散。

FRM 不能只用一个普通公共 FIR 滤波器解决，因为第 0、1、2……15 路的误差各不相同，而且拼接后误差具有 16 点周期性。
...
轮到ADC1的数据时，用第1套补偿系数
……
轮到ADC15的数据时，用第15套补偿系数
然后周期性重复
```

论文的补偿模型中，公共 FIR 滤波器 (C(j\\omega)) 用来校正 FRI，周期时变滤波器 (F(j\\omega)) 用来校正 FRM。

论文实验中，FRM 校准后：

- 子 ADC 间幅频响应相对误差由约 0.25 dB 降至 0.004 dB 以内；
- 群时延失配由约 135 ps 降至约 4 ps。

---

# 四、四个概念放在一起比较

|名称|主要回答的问题|是否影响正常采集|是否能跟踪温漂|典型方式|
| --------------| ------------------------------| -----------------: | ---------------: | --------------------------------|
|前台／预校准|什么时候测误差|通常需要暂停|较弱|标准信号、扫频、保存系数|
|后台校准|能否边工作边测误差|不暂停|可以|统计、相关、杂散检测、注入信号|
|后台实时校准|能否边测、边更新、边高速补偿|不暂停|可以|FPGA/ASIC 在线估计与流水补偿|
|频响误差校准|具体校准什么误差|两种模式都可以|取决于实现|FIR、PTV、FFT/IFFT、滤波器组|
...
但高速 ADC 的数据太快，普通串行滤波处理不过来，所以作者又解决了：
...
> **前台、后台是在讲“什么时候找到误差”；实时是在讲“能不能跟上数据”；频响校准是在讲“到底修正哪一种误差”。**
